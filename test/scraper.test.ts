...
import { fetchSubpageUrls, extractGpxLinks, downloadGpxFile } from '../src/scraper';
...
// fetchSubpageUrls
// ═════════════════════════════════════════════════════════════════════════════
describe('fetchSubpageUrls', () => {
  const LISTING_URL = 'https://kektura.hu/okt-szakaszok';
  const PATTERN     = /\/okt-szakasz\//i;

  test('returns absolute subpage URLs matching the pattern', async () => {
    axiosMock.onGet(LISTING_URL).reply(200, htmlWithLinks(
      'https://www.kektura.hu/okt-szakasz/okt-01',
      'https://www.kektura.hu/okt-szakasz/okt-02',
      'https://www.kektura.hu/other-page',
    ));
    const urls = await fetchSubpageUrls(LISTING_URL, PATTERN);
  ...
  test('resolves relative hrefs against the listing page origin', async () => {
    axiosMock.onGet(LISTING_URL).reply(200, htmlWithLinks('/okt-szakasz/okt-20'));
    const urls = await fetchSubpageUrls(LISTING_URL, PATTERN);
  ...
  test('deduplicates repeated links', async () => {
    ...
    expect(await fetchSubpageUrls(LISTING_URL, PATTERN)).toHaveLength(1);
  });

  test('returns empty array when no links match the pattern', async () => {
    axiosMock.onGet(LISTING_URL).reply(200, '<html><body><p>no links</p></body></html>');
    expect(await fetchSubpageUrls(LISTING_URL, PATTERN)).toEqual([]);
  });

  test('throws when the HTTP request fails', async () => {
    axiosMock.onGet(LISTING_URL).networkError();
    await expect(fetchSubpageUrls(LISTING_URL, PATTERN)).rejects.toThrow();
  });
});
...
// extractGpxLinks
// ═════════════════════════════════════════════════════════════════════════════
describe('extractGpxLinks', () => {
  const PAGE_URL = 'https://kektura.hu/okt-szakaszok';

  test('extracts GPX links from <a href> attributes', async () => {
    axiosMock.onGet(PAGE_URL).reply(200, htmlWithLinks(
  ...
  test('falls back to scanning raw HTML body for plain-text filenames', async () => {
  ...
  test('handles all three trail types (okt, ak, rpddk)', async () => {
    axiosMock.onGet(PAGE_URL).reply(200, htmlWithLinks(
      'okt_20_20251016.gpx',
      'ak_06_20250612.gpx',
  ...
  test('ignores links that do not match the GPX filename pattern', async () => {
    axiosMock.onGet(PAGE_URL).reply(200, htmlWithLinks(
...
describe('downloadGpxFile', () => {
  ...
  test('downloads GPX and passes it to the adapter', async () => {
    axiosMock.onGet(GPX_URL).reply(200, GPX_DATA);
    await downloadGpxFile(TRAIL, FILENAME, mockAdapter);
    expect(mockAdapter.writeGpx).toHaveBeenCalledWith(TRAIL, FILENAME, expect.any(Buffer));
  });

  test('passes correct trail and filename for all three trail types', async () => {
    const cases = [
      {
        trail: 'okt',
        filename: 'okt_01_20251107.gpx'   
    ...
    for (const {
      ...
      axiosMock.onGet(`https://test-gpx.example.com/gpx/${filename}`).reply(200, Buffer.from('<gpx/>'));
      await downloadGpxFile(trail, filename, mockAdapter);
  ...
  test('throws when the download request fails', async () => {
    ...
    await expect(downloadGpxFile(TRAIL, FILENAME, mockAdapter)).rejects.toThrow();
  });

  test('throws when the adapter writeGpx fails', async () => {
    axiosMock.onGet(GPX_URL).reply(200, GPX_DATA);
    (mockAdapter.writeGpx as jest.Mock).mockRejectedValueOnce(new Error('Write failed'));
    await expect(downloadGpxFile(TRAIL, FILENAME, mockAdapter)).rejects.toThrow('Write failed');
  });

  test('with LocalStorageAdapter: writes real file to disk', async () => {
    const tmpDir = makeTempDir();
    const localAdapter = new LocalStorageAdapter(tmpDir);
    axiosMock.onGet(GPX_URL).reply(200, GPX_DATA);
    await downloadGpxFile(TRAIL, FILENAME, localAdapter);
    expect(fs.existsSync(path.join(tmpDir, 'gpx', TRAIL, FILENAME))).toBe(true);
    fs.rmSync(tmpDir, {
      recursive: true,
      force: true,
    });
