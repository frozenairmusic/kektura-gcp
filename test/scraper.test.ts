...
describe('extractGpxLinks', () => {
  ...
  test('extracts GPX links from <a href> attributes', async () => {
    axiosMock.onGet(PAGE_URL).reply(200, htmlWithLinks('https://turistaterkepek.hu/gpx/okt_01_20251107.gpx',
  ...
  test('handles all three trail types (okt, ak, rpddk)', async () => {
    axiosMock.onGet(PAGE_URL).reply(200, htmlWithLinks('okt_20_20251016.gpx',
      'ak_06_20250612.gpx',
  ...
  test('ignores links that do not match the GPX filename pattern', async () => {
    axiosMock.onGet(PAGE_URL).reply(200, htmlWithLinks('unknown_01_20251107.gpx',
      'okt_1_20251107.gpx',
      'okt_01_2025110.gpx',
...
describe('downloadGpxFile', () => {
  ...
  test('throws when the download request fails', async () => {
    ...
    await expect(downloadGpxFile(TRAIL, FILENAME, mockAdapter,
    )).rejects.toThrow();
  });
  ...
  test('with LocalStorageAdapter: writes real file to disk', async () => {
    ...
    expect(fs.existsSync(path.join(tmpDir, 'gpx', TRAIL, FILENAME,
    ))).toBe(true);
    fs.rmSync(tmpDir, {
      recursive: true,
      force: true,
