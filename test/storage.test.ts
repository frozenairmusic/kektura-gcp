...
describe(
  'GcsStorageAdapter', () => {
    ...
    test(
      'readMetadata returns parsed JSON when file exists', async () => {
        const stored = {
          okt: {
            '01': {
    ...
    test(
      'readMetadata returns {} when file does not exist (404)', async () => {
        gcsMock.download.mockRejectedValueOnce(Object.assign(
          new Error('Not Found'), { code: 404 },
        ));
    ...
    test(
      'readMetadata re-throws non-404 errors', async () => {
        gcsMock.download.mockRejectedValueOnce(Object.assign(
          new Error('Server Error'), { code: 500 },
        ));
    ...
    test(
      'writeMetadata saves pretty-printed JSON with correct content-type', async () => {
        const metadata = {
          okt: {
            '20': {
    ...
    test(
      'writeMetadata propagates GCS errors', async () => {
    ...
    test(
      'writeGpx stores file at gpx/<trail>/<filename>', async () => {
        const data = Buffer.from('<gpx/>');
        await adapter.writeGpx(
          'okt', 'okt_01_20251107.gpx', data,
        );
        expect(gcsMock.file).toHaveBeenCalledWith('gpx/okt/okt_01_20251107.gpx');
        expect(gcsMock.save).toHaveBeenCalledWith(
          data, expect.objectContaining({ contentType: 'application/gpx+xml' }),
        );
      },
    );

    test(
      'writeGpx propagates GCS errors', async () => {
        gcsMock.save.mockRejectedValueOnce(new Error('Upload failed'));
        await expect(adapter.writeGpx(
          'okt', 'okt_01_20251107.gpx', Buffer.from(''),
        )).rejects.toThrow('Upload failed');
      },
    );

    test(
      'checkWritable saves and deletes a probe file', async () => {
    ...
    test(
      'checkWritable propagates save errors', async () => {
        gcsMock.save.mockRejectedValueOnce(new Error('Permission denied'));
...
describe(
  'LocalStorageAdapter', () => {
    ...
    test(
      'readMetadata returns {} when metadata.json does not exist', async () => {
        expect(await adapter.readMetadata()).toEqual({});
      },
    );

    test(
      'readMetadata returns saved metadata after writeMetadata', async () => {
        const metadata = {
          ak: {
            '01': {
    ...
    test(
      'writeMetadata persists pretty-printed JSON to disk', async () => {
        const metadata = {
          okt: {
            '01': {
              last_updated: '20251107',
    ...
    test(
      'writeGpx creates the file at outputDir/gpx/<trail>/<filename>', async () => {
        const data = Buffer.from('<gpx/>');
        await adapter.writeGpx(
          'rpddk', 'rpddk_01_20251008.gpx', data,
        );
        const filePath = path.join(
          tmpDir, 'gpx', 'rpddk', 'rpddk_01_20251008.gpx',
        );
    ...
    test(
      'writeGpx creates nested directories automatically', async () => {
        await adapter.writeGpx(
          'okt', 'okt_01_20251107.gpx', Buffer.from('<gpx/>'),
        );
    ...
    test(
      'checkWritable succeeds and leaves no .write-probe file behind', async () => {
        await adapter.checkWritable();
        expect(fs.existsSync(path.join(
          tmpDir, '.write-probe',
        ))).toBe(false);
...
describe(
  'createStorageAdapter', () => {
    ...
    test(
      'returns LocalStorageAdapter when LOCAL_OUTPUT_DIR is set', () => {
    ...
    test(
      'returns GcsStorageAdapter when GCS_BUCKET_NAME is set', () => {
    ...
    test(
      'prefers LOCAL_OUTPUT_DIR over GCS_BUCKET_NAME when both are set', () => {
        process.env.LOCAL_OUTPUT_DIR = tmpDir;
        process.env.GCS_BUCKET_NAME = 'my-bucket';
        expect(createStorageAdapter()).toBeInstanceOf(LocalStorageAdapter);
      },
    ...
    test(
      'throws when neither env var is set', () => {
