...
// ─── Module mocks ─────────────────────────────────────────────────────────────
jest.mock(
  '@google-cloud/functions-framework', () => ({ http: jest.fn() }),
);
...
describe(
  'syncGpxFiles handler', () => {
    ...
    function makeListingHtml(
      trail: string, filenames: string[],
    ): string {
      const subpageUrls = filenames.map((
        _, i,
      ) =>
        `https://www.kektura.hu/${trail}-szakasz/${trail}-${String(i + 1).padStart(
          2, '0',
        )}`);
    ...
    function stubGpxDownloads(...filenames: string[]): void {
      filenames.forEach(f => axiosMock.onGet(new RegExp(f.replace(
        '.', '\\.',
      ))).reply(
        200, Buffer.from('<gpx/>'),
      ));
    ...
    function mockTrailWithSubpages(
      trail: string, listingUrl: string, filenames: string[],
    ): void {
      axiosMock.onGet(listingUrl).reply(
        200, makeListingHtml(
          trail, filenames,
        ),
      );
      filenames.forEach((
        ...
        const sub = `https://www.kektura.hu/${trail}-szakasz/${trail}-${String(i + 1).padStart(
          2, '0',
        )}`;
        axiosMock.onGet(sub).reply(
          200, htmlWithLinks(filename),
        );
      });
    }
  ...
  function setupScrapeMocks({
    ...
    mockTrailWithSubpages(
      'okt',   'https://kektura.hu/okt-szakaszok',   oktLinks,
    );
    mockTrailWithSubpages(
      'ak',    'https://kektura.hu/ak-szakaszok',    akLinks,
    );
    mockTrailWithSubpages(
      'rpddk', 'https://kektura.hu/rpddk-szakaszok', rpddkLinks,
    );
    stubGpxDownloads(
      ...oktLinks, ...akLinks, ...rpddkLinks,
    );
  }

  // ── Missing storage configuration ────────────────────────────────────────
  test(
    'returns 500 when neither GCS_BUCKET_NAME nor LOCAL_OUTPUT_DIR is set', async () => {
      delete process.env.LOCAL_OUTPUT_DIR;
      const res = mockRes();
      await syncGpxFiles(
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
      );
  ...
  test(
    'returns 500 and aborts when storage write check fails', async () => {
      delete process.env.LOCAL_OUTPUT_DIR;
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
  ...
  test(
    'returns non-500 when LOCAL_OUTPUT_DIR is set', async () => {
      setupScrapeMocks({});
      const res = mockRes();
      await syncGpxFiles(
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
  ...
  // ── New GPX files ────────────────────────────────────────────────────────
  test(
    'adds new GPX files and writes them to the local output dir', async () => {
      setupScrapeMocks({
        oktLinks: [
          'okt_20_20251016.gpx',
          'okt_21_20251016.gpx',
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
      ...
      expect(fs.existsSync(path.join(
        tmpDir, 'gpx', 'okt', 'okt_20_20251016.gpx',
      ))).toBe(true);
      expect(fs.existsSync(path.join(
        tmpDir, 'gpx', 'okt', 'okt_21_20251016.gpx',
      ))).toBe(true);
  ...
  // ── Updated GPX file ─────────────────────────────────────────────────────
  test(
    'updates a GPX file when the scraped date is newer than stored', async () => {
      setupScrapeMocks({
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
  ...
  // ── Unchanged GPX file ───────────────────────────────────────────────────
  test(
    'skips GPX files whose date matches metadata and does not overwrite them', async () => {
      setupScrapeMocks({
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
      ...
      expect(body.summary.added).toBe(0);
      expect(fs.existsSync(path.join(
        tmpDir, 'gpx', 'okt', 'okt_01_20251107.gpx',
      ))).toBe(false);
    },
  );
  ...
  test(
    'records listing-page errors but continues processing other targets', async () => {
      fs.writeFileSync(
        path.join(
          tmpDir, 'metadata.json',
        ), '{}',
      ...
      axiosMock.onGet('https://kektura.hu/ak-szakaszok').reply(
        200,
        htmlWithLinks('https://www.kektura.hu/ak-szakasz/ak-01'),
      );
      axiosMock.onGet('https://www.kektura.hu/ak-szakasz/ak-01').reply(
        200,
        htmlWithLinks('ak_01_20231109.gpx'),
      );
      axiosMock.onGet('https://kektura.hu/rpddk-szakaszok').reply(
        200, '<html></html>',
      );
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
  ...
  test(
    'records GPX download errors but still processes remaining segments', async () => {
      fs.writeFileSync(
        path.join(
          tmpDir, 'metadata.json',
        ), '{}',
      );
      axiosMock.onGet('https://kektura.hu/okt-szakaszok').reply(
        200, htmlWithLinks(
          'https://www.kektura.hu/okt-szakasz/okt-01',
          'https://www.kektura.hu/okt-szakasz/okt-02',
        ),
      );
      axiosMock.onGet('https://www.kektura.hu/okt-szakasz/okt-01').reply(
        200, htmlWithLinks('okt_01_20251107.gpx'),
      );
      axiosMock.onGet('https://www.kektura.hu/okt-szakasz/okt-02').reply(
        200, htmlWithLinks('okt_02_20251016.gpx'),
      );
      axiosMock.onGet('https://kektura.hu/ak-szakaszok').reply(
        200, '<html></html>',
      );
      ...
      axiosMock.onGet(/okt_02_20251016\.gpx/).reply(
        200, Buffer.from('<gpx/>'),
      );
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
  ...
  // ── Mixed across trails ──────────────────────────────────────────────────
  test(
    'correctly categorises segments across multiple trails', async () => {
      setupScrapeMocks({
        oktLinks: ['okt_01_20251107.gpx'],
...
mockReq() as unknown as ff.Request, res as unknown as ff.Response,
  ...
  // ── Response shape ───────────────────────────────────────────────────────
  test(
    'returns a well-formed response body', async () => {
