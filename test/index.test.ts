...
gcsMock.bucket.mockReturnValue({
  file: gcsMock.file,
});
MockedStorage.mockImplementation(() => ({
  bucket: gcsMock.bucket,
...
describe('syncGpxFiles handler', () => {
  ...
  function setupScrapeMocks({
    ...
    mockTrailSubpages('okt',   oktLinks,
    );
    mockTrailSubpages('ak',    akLinks,
    );
    mockTrailSubpages('rpddk', rpddkLinks,
    );
    // Catch-all: remaining subpages return empty HTML (no GPX links)
    axiosMock.onGet(/kektura\.hu/).reply(200, '<html></html>',
    );
  ...
  test('adds new GPX files and writes them to the local output dir', async () => {
    ...
    expect(fs.existsSync(path.join(tmpDir, 'gpx', 'okt', 'okt_20_20251016.gpx',
    ))).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, 'gpx', 'okt', 'okt_21_20251016.gpx',
    ))).toBe(true);
    const saved = JSON.parse(fs.readFileSync(path.join(tmpDir, 'metadata.json',
    ), 'utf8',
    ));
  ...
  test('updates a GPX file when the scraped date is newer than stored', async () => {
    ...
    const body = res._body as { summary: { updated: number;
      added: number } };
  ...
  test('skips GPX files whose date matches metadata and does not overwrite them', async () => {
    ...
    const body = res._body as { summary: { unchanged: number;
      added: number;
      updated: number } };
    expect(body.summary.unchanged).toBe(1);
    expect(body.summary.added).toBe(0);
    expect(fs.existsSync(path.join(tmpDir, 'gpx', 'okt', 'okt_01_20251107.gpx',
    ))).toBe(false);
  });

  // ── Listing-page fetch error resilience ────────────────────────────────
  test('records subpage errors but continues processing other targets', async () => {
    fs.writeFileSync(path.join(tmpDir, 'metadata.json',
    ), '{}',
    );
    ...
    const body = res._body as { summary: { errors: number;
      added: number } };
