...
6. **Analyses** each downloaded GPX — groups stamp points by location, finds nearest track points, and computes distance / elevation gain / elevation loss between consecutive stamp-point groups. Each section records `track_from` and `track_to` as zero-based indices into the GPX `<trkpt>` array, so Android can slice the polyline directly without re-scanning the track.
...
- **Tests** — Jest + ts-jest + axios-mock-adapter (67 tests, 0 live network calls)
